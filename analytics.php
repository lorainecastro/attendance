...
// OpenAI API Key (from ai.js)
define('OPENAI_API_KEY', ''); // Replace with your actual key
...
// Function to generate AI recommendation using OpenAI API
function generateAIRecommendation($pdo, $class_id, $lrn, $attendance_rate, $risk_level, $forecast_rate, $is_college) {
    $period = getHistoricalPeriod();
    $period_start = $period['forecast_start'];

    // Check if recommendation exists for the current period
    $stmt = $pdo->prepare("
    ...
    if ($risk_level === 'no risk') {
        $recommendation = "Encourage consistent attendance with positive reinforcement.";
    } else {
        // Construct prompt for OpenAI
        $trend = $forecast_rate >= $attendance_rate ? 'improving' : 'declining';
        $prompt = "A student has an attendance rate of {$attendance_rate}%, a risk level of '{$risk_level}'" .
          ($is_college ? " (college student)" : " (Kinder to Grade 12)") .
          ", and a forecasted attendance rate of {$forecast_rate}% next month, indicating an {$trend} trend. " .
          "Provide a concise recommendation for the teacher to improve or maintain the student's attendance for the period {$period['forecast_start']} to {$period['forecast_end']}.";
        try {
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                ...
                $recommendation = "Unable to generate recommendation due to JSON decode error.";
            } elseif (isset($data['choices'][0]['message']['content'])) {
                $recommendation = trim($data['choices'][0]['message']['content']);
            } else {
                error_log("OpenAI API response error: " . json_encode($data));
                $recommendation = "Unable to generate recommendation due to API error.";
            }
        } catch (Exception $e) {
            error_log("OpenAI API error: " . $e->getMessage());
            $recommendation = "Error fetching AI recommendation.";
        }
    }
    ...
    try {
        $stmt = $pdo->prepare("
            INSERT INTO ai_recommendations (class_id, lrn, recommendation, period_start)
            VALUES (:class_id, :lrn, :recommendation, :period_start)
        ...
        error_log("Database error inserting AI recommendation: " . $e->getMessage());
    }
...
try {
    ...
    foreach ($classes_db as $class) {
        ...
        foreach ($students as $student) {
            ...
            $ai_recommendation = generateAIRecommendation($pdo, $class['class_id'], $student['lrn'], $current_rate, $riskLevel, $avg_forecast, $isCollege);
        ...
        $classes[] = [
            ...
            'sectionName' => isset($class['section_name']) ? $class['section_name'] : 'Unknown',
            'subject' => isset($class['subject_name']) ? $class['subject_name'] : 'Unknown',
            'gradeLevel' => isset($class['grade_level']) ? $class['grade_level'] : '',
        ...
        foreach ($student_data as $s) {
            $absence_threshold = $isCollege ? 3 : 14;
            ...
            <select class="selector-select" id="class-filter">
                <?php if (!empty($classes)): ?>
                    <option value="<?php echo htmlspecialchars($classes[0]['id']); ?>">
                        <?php echo htmlspecialchars((isset($classes[0]['gradeLevel']) ? $classes[0]['gradeLevel'] . ' – ' : '') . $classes[0]['sectionName'] . ' (' . $classes[0]['subject'] . ')'); ?>
                    </option>
                <?php else: ?>
    ...
    <script>
        ...
        function initializeFilters() {
            ...
            classes.forEach(cls => {
                if (!uniqueClasses.has(cls.id)) {
                    ...
                    option.value = cls.id;
                    option.textContent = `${cls.gradeLevel ? cls.gradeLevel + ' – ' : ''}${cls.sectionName} (${cls.subject})`;
        ...
        function updateAtRiskTable() {
            ...
            classes.forEach(cls => {
                if (!selectedClassId || cls.id == selectedClassId) {
                    const isCollege = cls.gradeLevel && cls.gradeLevel.includes('College');
                    const absenceThreshold = isCollege ? 3 : 14;

                    cls.students.forEach(student => {
                        if (student.totalAbsences >= absenceThreshold) {
                            hasAtRisk = true;
                            const row = document.createElement('tr');
                            const riskText = isCollege ? (student.riskLevel === 'Running for Drop Out' ? 'Running for Drop Out' : 'No Risk') : student.riskLevel.charAt(0).toUpperCase() + student.riskLevel.slice(1);
                            row.innerHTML = `
                                <td>${cls.gradeLevel ? cls.gradeLevel + ' - ' : ''}${cls.sectionName} (${cls.subject})</td>
                                <td>${student.lastName}, ${student.firstName} ${student.middleName || ''}</td>
                                <td class="risk-${student.riskLevel.replace(/\s+/g, '-').toLowerCase()}">${student.totalAbsences} - ${riskText}</td>
                            `;
        ...
        function showStudentPrediction(studentId) {
            ...
            document.getElementById('student-details').innerHTML = `
                ...
                <div class="detail-item">
                    <strong>Total Absences:</strong> ${student.totalAbsences}${isCollege ? ` - ${student.riskLevel}` : ` - ${student.riskLevel.charAt(0).toUpperCase() + student.riskLevel.slice(1)}`}
                </div>
                ${isCollege ? '' : `
                <div class="detail-item risk-${student.riskLevel.replace(/\s+/g, '-').toLowerCase()}">
                    <strong>Risk Level:</strong> ${student.riskLevel.charAt(0).toUpperCase() + student.riskLevel.slice(1)}
                </div>
                `}
            `;
            
            document.getElementById('student-recommendation-text').textContent = student.aiRecommendation;
            ...
            metricsTable.innerHTML = `
                ...
                <tr>
                    ...
                    <td class="risk-${student.riskLevel.replace(/\s+/g, '-').toLowerCase()}">${student.totalAbsences}${isCollege ? ` - ${student.riskLevel}` : ` - ${student.riskLevel.charAt(0).toUpperCase() + student.riskLevel.slice(1)}`}</td>
                </tr>
            `;
        ...
        function exportChart() {
            ...
            link.download = `attendance_forecast_${selectedClass.sectionName}.png`;
        ...
        classFilter.addEventListener('change', () => {
            ...
            if (studentFilter.value) {
                showStudentPrediction(studentFilter.value);
            } else {
                document.getElementById('student-prediction-card').style.display = 'none';
                document.getElementById('attendance-status-title').textContent = 'Attendance Status Distribution';
            }
        ...
        studentSearch.addEventListener('input', () => {
            updateStudentFilter(studentSearch.value);
        });
        ...
        document.addEventListener('DOMContentLoaded', () => {
            if (classes.length > 0) {
                ...
                console.warn('No classes available to initialize');
                document.getElementById('student-prediction-card').style.display = 'none';
                document.getElementById('current-attendance-rate').textContent = 'No data';
                document.getElementById('predicted-attendance').textContent = 'No data';
