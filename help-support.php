...
<head>
    ...
    <style>
        ...
        * {
        ...
        .excel-format tr:nth-child(even) td {
            background-color: var(--inputfield-color);
        }

        .excel-format tr:hover td {
            background-color: var(--inputfieldhover-color);
        }
        ...
        @media (max-width: 768px) {
            .faq .grid, .manual .grid {
                grid-template-columns: 1fr; /* back to 1 column on tablets/phones */
            }
        }
...
<body>
    ...
    <section id="faq" class="faq">
        <div class="excel-format">
            <h2>Excel Import Format for Class Management</h2>
            <table>
                <thead>
                    <tr>
                        <th>LRN</th>
                ...
                <tbody>
                    <tr>
                        ...
                        <td>Ava</td>
                        <td>Lou</td>
                        <td>ava.brown@email.com</td>
                        <td>Female</td>
                        <td>2006-02-27</td>
                        <td>Grade 11</td>
                        <td>278 Example St</td>
                        <td>David Brown</td>
    ...
    <section id="manuals" class="manual">
        ...
        <div class="grid">
            ...
            <div class="manual-item" onclick="this.classList.toggle('active')">
                <h3>Attendance Marking User Manual <span class="arrow"> ❯ </span></h3>
                <div class="content">
                    <h4>1. Marking Attendance</h4>
                    <p>To mark attendance for a class:<br>1. Select Class:<br>  - Choose the Grade Level, Section, and Subject from the dropdown menus.<br>  - Ensure the selected date is today, as past dates cannot be modified.<br>2. View Students:<br>  - A table displays students for the selected class, showing:<br>    - LRN<br>    - Name<br>    - Photo<br>    - Status<br>    - Time Checked<br>3. Mark Attendance:<br>  - For individual students, select Present, Absent, or Late from the status dropdown.<br>  - For multiple students, use checkboxes to select students and apply bulk actions (Present, Absent, Late) via the bulk action dropdown.<br>  - Click Mark All Present to set all non-QR-scanned students as Present.<br>4. Submit:<br>  - Click Submit Attendance to save changes.<br>  - A success notification appears upon successful submission; otherwise, an error message displays.</p>
                    <h4>2. Using the QR Code Scanner</h4>
                    <p>To mark attendance using a QR code scanner:<br>1. Enable Scanner:<br>  - Click Scan QR Code to activate the camera or USB scanner.<br>  - This feature is only available for today’s date and classes with an active schedule.<br>  - Ensure camera permissions are granted or the USB scanner is properly connected.<br>2. Scan QR Code:<br>  - Point the camera at the student’s QR code or use a USB scanner.<br>  - The system automatically detects the LRN and marks the student as:<br>    - Present (if scanned within the grace period)<br>    - Late (if scanned after the grace period)<br>3. Notifications:<br>  - A notification confirms the scan and the assigned status.<br>  - Parents receive an email for Present QR scans within the grace period.<br>4. Stop Scanner:<br>  - Click Stop Scanner to deactivate the camera or scanner.</p>
                    <h4>3. Viewing Attendance</h4>
                    <p>• Statistics:<br>  - View the following for the filtered class list:<br>    - Total Students<br>    - Present Count<br>    - Absent Count<br>    - Attendance Percentage<br>• Schedule Info:<br>  - Displays the class schedule and grace period (if applicable) for the selected date and class.</p>
                    <h4>4. Key Features</h4>
                    <p>• Grace Period:<br>  - QR scans within the grace period mark students as Present; scans after the grace period mark students as Late.<br>  - A grace period countdown is displayed when active.<br>• Locked QR Records:<br>  - Attendance records marked via QR code scanning cannot be manually edited.<br>• Clear Filters:<br>  - Reset all filters to default (today’s date, first available grade/section/subject) using the Clear Filters button.</p>
                    <h4>5. Restrictions</h4>
                    <p>• Attendance can only be marked for the current date with an active class schedule.<br>• Past date attendance records cannot be edited.<br>• QR scanning requires:<br>  - A selected class<br>  - Today’s date<br>  - An active class schedule</p>
                    <h4>6. Troubleshooting</h4>
                    <p>• No Schedule Found:<br>  - Ensure the selected class has a schedule defined for the selected date in the Class Management settings.<br>• QR Scanner Issues:<br>  - Check camera permissions or verify the USB scanner is properly connected.<br>  - Ensure the correct class and today’s date are selected.<br>  - Confirm the class has an active schedule.<br>• Email Not Sent:<br>  - Verify that the student’s parent email is added in the student’s profile.</p>
                </div>
            </div>
            <div class="manual-item" onclick="this.classList.toggle('active')">
                <h3>Overall Attendance User Manual <span class="arrow"> ❯ </span></h3>
                <div class="content">
                    <h4>1. Viewing Attendance</h4>
                    <p>To view attendance records:<br>1. Select Filters:<br>  - Date: Choose a date using the date selector (from the earliest recorded attendance to today).<br>  - Grade Level: Select a specific grade level or "All Grade Levels" from the dropdown.<br>  - Section: Choose a section or "All Sections" (options update based on selected grade level).<br>  - Subject: Select a subject or "All Subjects" (options update based on grade and section).<br>  - Status: Filter by attendance status (All, Present, Absent, Late).<br>  - Search: Enter a student’s LRN or name to filter the list.<br>2. View Results:<br>  - A table displays the filtered students with the following columns:<br>    - Photo<br>    - LRN<br>    - Student Name<br>    - Status<br>    - Time Checked<br>  - Status badges are color-coded:<br>    - Present: Green<br>    - Absent: Red<br>    - Late: Yellow<br>    - None: Gray<br>  - Time Checked: Shows the date and time (in Asia/Manila timezone) when attendance was recorded.</p>
                    <h4>2. Understanding Statistics</h4>
                    <p>The system provides the following statistics for the filtered list:<br>• Total Students: Number of students in the filtered list.<br>• Present Count: Number of students marked as Present or Late.<br>• Absent Count: Number of students marked as Absent.<br>• Attendance Percentage: Percentage of students marked Present or Late out of the total filtered students.</p>
                    <h4>3. Key Features</h4>
                    <p>• Filtering: Combine date, grade level, section, subject, status, and search criteria to narrow down results.<br>• Clear Filters: Reset all filters to default settings (today’s date, all grades/sections/subjects) using the Clear Filters button.</p>
                    <h4>4. Restrictions</h4>
                    <p>• View-Only: Attendance records cannot be edited on this page. Use the Attendance Marking page to make changes.<br>• Date Limit: Future dates cannot be selected; the maximum date is today.<br>• Data Availability: Only attendance data from the earliest recorded date for your classes is displayed.</p>
                    <h4>5. Troubleshooting</h4>
                    <p>• No Students Displayed:<br>  - Check if filters are too restrictive. Use the Clear Filters button to broaden results.<br>  - Ensure students are enrolled in the selected class.<br>• Missing Classes:<br>  - Verify that you are assigned to classes and that they are not archived.<br>  - Check class assignments in the Class Management section.<br>• Date Issues:<br>  - Confirm that attendance records exist for the selected date.</p>
                </div>
            </div>
            <div class="manual-item" onclick="this.classList.toggle('active')">
                <h3>Analytics & Predictions User Manual <span class="arrow"> ❯ </span></h3>
                <div class="content">
                    <h4>1. Overview</h4>
                    <p>The dashboard displays:<br>• Class and student filters<br>• Attendance statistics cards<br>• ARIMA time series forecast chart<br>• Attendance status distribution chart<br>• At-risk students table<br>• Individual student analytics (when a student is selected)</p>
                    <h4>2. Filtering Data</h4>
                    <p>To filter attendance data:<br>1. Select Class:<br>  - Use the Class Filter dropdown to select a class (e.g., "Grade 10 – Section A (Mathematics)").<br>  - Default: The first available class is selected.<br>2. Search Students:<br>  - Enter a student’s LRN or name in the Search input field (case-insensitive, supports partial matches).<br>3. Select Student:<br>  - Use the Student Filter dropdown to select a specific student or "All Students."<br>  - The dropdown updates based on the selected class and search term.<br>4. Update Forecast:<br>  - Click the Update Forecast button (with refresh icon) to reload data and refresh forecasts.<br>5. Clear Filters:<br>  - Click the Clear button (with X icon) to reset all filters to default (first class, all students, no search term).</p>
                    <h4>3. Viewing Attendance Statistics</h4>
                    <p>The dashboard displays key statistics in cards:<br>• Current Attendance Rate:<br>  - Shows the current attendance rate for the selected class (percentage of Present or Late records).<br>  - Includes a trend indicator (up, down, or stable) compared to the previous period.<br>• Predicted Next Month:<br>  - Displays the forecasted attendance rate for the next month using the ARIMA model.<br>• At-Risk Students:<br>  - Shows the number of students at risk of dropping out based on absence thresholds (more than 3 absences for college, more than 14 for K-12).<br>  - Includes a trend indicator (up, down, or stable) compared to the previous period.</p>
                    <h4>4. Viewing Attendance Forecasts</h4>
                    <p>The ARIMA Time Series Forecast chart displays:<br>• Historical Data: Attendance rates for the selected class or student from the earliest recorded date to the current period.<br>• Forecasted: Predicted attendance rates for the next 30 days, shown as a dashed line.<br>• Chart Details:<br>  - Y-axis: Attendance rate (0% to 100%).<br>  - X-axis: Dates (historical and forecasted).<br>  - Blue line: Historical attendance data.<br>  - Red dashed line: Forecasted attendance rate.</p>
                    <h4>5. Analyzing At-Risk Students</h4>
                    <p>The At-Risk Students table lists students with excessive absences:<br>• Columns:<br>  - Class: Grade level, section, and subject (e.g., "Grade 10 - Section A (Mathematics)").<br>  - Name: Student’s full name.<br>  - Total Absences: Number of absences and risk level (e.g., "15 - High" for K-12, "5 - Running for Drop Out" for college).<br>• Risk Levels (K-12):<br>  - Low: 1–13 absences<br>  - Medium: 14–26 absences<br>  - High: 27–39 absences<br>  - Running for Drop Out: 40+ absences<br>• Risk Levels (College):<br>  - No Risk: 0–2 absences<br>  - Running for Drop Out: 3+ absences<br>• Note: The table updates based on the selected class filter. If no students are at risk, it displays "No data available."</p>
                    <h4>6. Viewing Individual Student Analytics</h4>
                    <p>When a student is selected from the Student Filter:<br>• Prediction Card:<br>  - Displays detailed analytics for the selected student, including:<br>    - Class: Subject and section.<br>    - LRN: Student’s Learner Reference Number.<br>    - Student: Full name.<br>    - Current Attendance Rate: Current attendance percentage.<br>    - Predicted Next Month: Forecasted attendance rate for the next month.<br>    - Total Absences: Number of absences and risk level.<br>    - Risk Level (K-12 only): Low, Medium, High, or Running for Drop Out.<br>• Individual Forecast Chart:<br>  - Shows historical and forecasted attendance rates for the student.<br>  - Blue line: Historical data.<br>  - Red dashed line: Forecasted rate.<br>• Personal Analytics Table:<br>  - Lists current metrics (Attendance Rate, Total Absences with risk level).<br>• Forecast Metrics Table:<br>  - Shows the forecasted attendance rate for the next month.<br>• AI Recommendation:<br>  - Displays an AI-generated recommendation (via OpenAI) to improve or maintain the student’s attendance.<br>  - Example: "Encourage consistent attendance with positive reinforcement" (for no risk).<br><strong>Note:</strong> The Attendance Status Distribution chart updates to show the selected student’s Present, Absent, and Late counts with percentages.</p>
                    <h4>7. Key Features</h4>
                    <p>• Class and Student Filters: Narrow down data by class, student, or search term.<br>• ARIMA Forecasting: Predicts attendance rates for the next 30 days based on historical data.<br>• Attendance Status Distribution: Visualizes Present (green), Absent (red), and Late (yellow) counts in a pie chart.<br>• At-Risk Analysis: Identifies students with excessive absences based on grade-specific thresholds.<br>• AI Recommendations: Provides tailored suggestions for improving student attendance using OpenAI.<br>• Responsive Design: Adapts to various screen sizes (desktop, tablet, mobile).</p>
                    <h4>8. Restrictions</h4>
                    <p>• Data Availability: Analytics require historical attendance data. If no data exists, charts and tables display "No data available."<br>• Class Assignment: Only classes assigned to the teacher and not archived are displayed.<br>• Date Range: Historical data starts from the earliest recorded attendance date; forecasts are limited to 30 days from the current period.<br>• View-Only: Attendance data cannot be edited on this page. Use the Attendance Marking page for modifications.</p>
                    <h4>10. Troubleshooting</h4>
                    <p>• No Data Displayed:<br>  - Verify that you are assigned to active classes in the Class Management section.<br>  - Ensure attendance records exist for the selected class.<br>  - Clear filters to broaden the data range.<br>• Charts Not Rendering:<br>  - Check browser console for JavaScript errors.<br>  - Ensure the browser supports Chart.js (use a modern browser like Chrome, Firefox, or Edge).<br>• At-Risk Students Not Showing:<br>  - Confirm that the class has students with absences exceeding the threshold (more than 3 for college, more than 14 for K-12).<br>  - Check if the class is archived; unarchive it in the Archived Classes section if needed.</p>
                </div>
            </div>
            <div class="manual-item" onclick="this.classList.toggle('active')">
                <h3>Reports User Manual <span class="arrow"> ❯ </span></h3>
                <div class="content">
                    <h4>1. Viewing Reports</h4>
                    <p>To generate and view reports:<br>1. Select Filters:<br>  - Class: Choose a specific class (e.g., "Grade 10 - Section A (Math)") from the dropdown or select "All Classes."<br>  - Student: Select a student by LRN or name using the search function, or choose "All Students."<br>  - Date Range: Set the "Date From" and "Date To" fields (defaults to the last month to today).<br>  - Report Type: Choose one of the following:<br>    - Student Attendance History<br>    - Attendance per Class<br>    - Perfect Attendance Recognition<br>2. Generate Report:<br>  - Click Generate Report to display the report based on your selected filters.<br>  - The report will appear in a table below the filter controls, with a title reflecting the selected report type.</p>
                    <h4>2. Report Types</h4>
                    <p>The system offers three types of reports, each with specific columns and details:<br><strong>2.1 Student Attendance History</strong><br>• Columns: Class, LRN, Name, Status (Present, Absent, Late), Time Checked<br>• Details: Displays individual attendance records with color-coded status badges:<br>  - Green: Present<br>  - Yellow: Late<br>  - Red: Absent<br><strong>2.2 Attendance per Class</strong><br>• Columns: Class, Total Students, Present, Absent, Late, Average Attendance<br>• Details: Provides class-level statistics. The average attendance is calculated as the percentage of Present records out of the total attendance records.<br><strong>2.3 Perfect Attendance Recognition</strong><br>• Columns: Class, LRN, Name, Status (Recognized/Not Recognized), Attendance Issue Summary, Adjusted Attendance Record<br>• Details: Identifies students with perfect attendance (no absences or lates, marked as Recognized with a green badge) or those with attendance issues (Not Recognized, red badge). Adjusted records reflect late-to-absent rules (e.g., multiple lates counted as absences based on class settings).</p>
                    <h4>3. Exporting Reports</h4>
                    <p>To export a report:<br>1. Select Format:<br>  - Choose either PDF or Excel from the "Export Format" dropdown.<br>2. Export:<br>  - Click Export Report to generate and download the report in the chosen format.<br>  - PDF: Includes a styled table with colored status badges and a header displaying the report title and date range.<br>  - Excel: Features formatted headers and color-coded status cells, with LRNs formatted as numbers.<br>3. Download:<br>  - The file will download automatically with a name like student-report-YYYY-MM-DD_HH-mm-ss.pdf or .xlsx.</p>
                    <h4>4. Key Features</h4>
                    <p>• Statistics Cards:<br>  - Total Classes: Displays the number of active classes assigned to the teacher.<br>  - Total Students: Shows the total number of students across all classes.<br>  - Late (Today): Indicates the number of students marked Late for the current day.<br>  - Absent (Today): Indicates the number of students marked Absent for the current day.<br>• Filtering: Combine class, student, date range, and report type to create customized reports.<br>• Search: Quickly filter students by LRN or name.<br>• Late-to-Absent Rule: In Perfect Attendance reports, converts lates to absences based on class-specific settings (e.g., 3 lates = 1 absent).</p>
                    <h4>5. Restrictions</h4>
                    <p>• Data Availability: Reports require existing attendance data. If no records are found for the selected class, student, or date range, the table will be empty.<br>• Date Range: Limited to dates with recorded attendance; future dates are not supported.</p>
                    <h4>6. Troubleshooting</h4>
                    <p>• No Data in Report:<br>  - Verify that the selected class, student, or date range has associated attendance records.<br>  - Adjust filters as needed.<br>• Export Fails:<br>  - Check browser permissions to ensure downloads are allowed.<br>  - Try using a different browser if the issue persists.<br>• Incorrect Late-to-Absent Counts:<br>  - Confirm the class’s late-to-absent rule in the schedule settings.<br>  - Ensure the rule (e.g., 3 lates = 1 absent) is correctly configured.<br>• Student Not Found:<br>  - Ensure the student is enrolled in the selected class.<br>  - Verify that the search term (LRN or name) matches the student’s details.</p>
                </div>
            </div>
            <div class="manual-item" onclick="this.classList.toggle('active')">
                <h3>Archived Classes User Manual <span class="arrow"> ❯ </span></h3>
                <div class="content">
                    <h4>1. Viewing Archived Classes</h4>
                    <p>To view archived classes:<br>1. Navigate:<br>  - Access the Archived Classes page in the Student Attendance System to view all classes marked as archived.<br>2. Class Cards:<br>  - Each archived class is displayed as a card containing the following details:<br>    - Grade & Section: Class grade level and section name (e.g., "Grade 10 - Section A").<br>    - Subject & Code: Subject name and code (e.g., "Mathematics, MATH101").<br>    - Room: Assigned room or "No room specified."<br>    - Total Students: Number of students enrolled in the class.<br>    - Late-to-Absent: Number of late marks counted as one absence (e.g., 3 lates = 1 absence).<br>    - Grace Period: Minutes allowed for late arrivals (e.g., 15 minutes).<br>    - Schedule: Days and times (e.g., "Monday: 8:00 AM - 9:00 AM (Grace: 15 min)").<br>    - Status: Marked as "Archived" with a red badge.</p>
                    <h4>2. Key Features</h4>
                    <p>• Statistics Cards:<br>  - Total Classes Archived: Displays the number of archived classes assigned to the teacher.<br>  - Total Students Archived: Shows the total number of students across all archived classes.<br>  - Total Unique Students: Indicates the number of distinct students in archived classes.<br>• View Class Details:<br>  - Click the View button on a class card to open a modal displaying:<br>    - Grade level, section, subject, subject code, room, total students, late-to-absent marks, grace period, and schedule.<br>• View Students:<br>  - Click the Students button on a class card to open a modal with a table listing:<br>    - LRN, last name, first name, middle name, email, gender, date of birth, grade level, address, parent name, parent email, emergency contact, photo, QR code.<br>• Unarchive Class:<br>  - Click the Unarchive button on a class card.<br>  - Confirm the action in the prompt to restore the class to active status.<br>  - Once unarchived, the class will be available in other system sections.</p>
                    <h4>3. Restrictions</h4>
                    <p>• View-Only:<br>  - Archived classes cannot be edited. To modify class details, you must first unarchive the class.<br>• Data Availability:<br>  - If no classes are archived, the system will display a message: "No archived classes found."</p>
                </div>
            </div>
            <div class="manual-item" onclick="this.classList.toggle('active')">
                <h3>Profile Page User Manual <span class="arrow"> ❯ </span></h3>
                <div class="content">
                    <h4>1. Updating Profile Information</h4>
                    <p>To update your profile details:<br>• Navigate to the Profile Information tab.<br>• When updating your profile:<br>The following fields are required:<br>  - First name<br>  - Last name<br>  - Username<br>The following fields are optional:<br>  - Institution<br>  - Profile picture<br>• Click Save Changes to apply your updates.<br><strong>Note:</strong> The email address field is read-only to maintain account integrity.</p>
                    <h4>2. Changing Your Username</h4>
                    <p>To update your username:<br>• Enter a new username in the Username field in the Profile Information tab.<br>• The system will verify if the username is unique.<br>• If the username is already in use, you will see an error message: "Username is already taken."<br>• Choose a different, unique username and retry.</p>
                    <h4>3. Supported Image Formats for Profile Pictures</h4>
                    <p>The system supports the following image file formats for profile pictures:<br>• JPG<br>• JPEG<br>• PNG<br>• GIF<br>Other file formats are not supported and will result in an error if uploaded.</p>
                    <h4>4. Optional Profile Picture Upload</h4>
                    <p>Uploading a profile picture is optional:<br>• You may choose to upload a new profile picture in a supported format.<br>• If you do not upload a profile picture, the system will retain your current picture or use the default image if none is set.</p>
                    <h4>5. Changing Your Password</h4>
                    <p>To update your password:<br>• Go to the Password Management tab.<br>• Enter the following:<br>  - Your current password.<br>  - Your new password.<br>  - The confirmation of your new password (must match the new password).<br>• Click Change Password to save the new password.<br><strong>Note:</strong> Ensure the new password and confirmation match to avoid errors.</p>
                    <h4>6. Previewing a New Profile Picture</h4>
                    <p>To preview a new profile picture:<br>• Select a new image file in the profile picture field.<br>• The system will display a preview of the selected image in the profile image section.<br>• Review the preview and click Save Changes to confirm, or select a different image if needed.</p>
                </div>
            </div>
        </div>
    </section>
