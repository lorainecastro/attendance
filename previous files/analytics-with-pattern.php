...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Monthly Attendance Calendar Styles */
        .monthly-attendance {
            display: flex;
...
<body>
    ...
    <div class="pattern-table">
        ...
        <h3>Monthly Attendance Patterns</h3>
        <div class="table-responsive">
            <table>
                <thead>
                    <tr>
                        <th>Student</th>
                        <th>Attendance Calendar</th>
                    </tr>
                </thead>
                <tbody id="monthly-attendance"></tbody>
            </table>
        </div>
    </div>
    ...
    <script>
        ...
        // Generate monthly attendance pattern for specified month
        function generateMonthlyAttendancePattern(attendanceRate, absenceCount, year = 2025, month = 6) {
            const daysInMonth = new Date(year, month + 1, 0).getDate();
            const pattern = [];
            const fridays = [];

            // Identify Fridays in the month
            for (let day = 1; day <= daysInMonth; day++) {
                const date = new Date(year, month, day);
                if (date.getDay() === 5) { // Friday is 5
                    fridays.push({ date: `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`, day });
                }
            }

            // Ensure specified number of Fridays are marked as Absent
            const absentFridays = Math.min(absenceCount, fridays.length);
            const selectedFridays = fridays.sort(() => Math.random() - 0.5).slice(0, absentFridays);

            // Generate pattern for each day
            for (let day = 1; day <= 35; day++) { // 35 to fill 5x7 grid
                if (day <= daysInMonth) {
                    const dateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
                    const isFriday = new Date(year, month, day).getDay() === 5;
                    const isAbsentFriday = isFriday && selectedFridays.some(f => f.date === dateStr);
                    pattern.push({
                        date: dateStr,
                        status: isAbsentFriday ? 'Absent' : (Math.random() < (attendanceRate / 100) ? 'Present' : 'Absent')
                    });
                } else {
                    pattern.push({ date: '', status: 'Empty' });
                }
            }

            // Adjust attendance to match rate if needed
            const actualAbsentCount = pattern.filter(p => p.status === 'Absent').length;
            const expectedAbsentCount = Math.floor(daysInMonth * (1 - attendanceRate / 100));
            let adjustments = actualAbsentCount - expectedAbsentCount;

            if (adjustments > 0) {
                // Too many absences, convert some to Present
                let nonFridayAbsents = pattern.filter(p => p.status === 'Absent' && new Date(p.date).getDay() !== 5);
                for (let i = 0; i < adjustments && nonFridayAbsents.length > 0; i++) {
                    const idx = pattern.findIndex(p => p.date === nonFridayAbsents[0].date);
                    pattern[idx].status = 'Present';
                    nonFridayAbsents = nonFridayAbsents.slice(1);
                }
            } else if (adjustments < 0) {
                // Too few absences, convert some to Absent
                let nonFridayPresents = pattern.filter(p => p.status === 'Present' && new Date(p.date).getDay() !== 5);
                for (let i = 0; i < -adjustments && nonFridayPresents.length > 0; i++) {
                    const idx = pattern.findIndex(p => p.date === nonFridayPresents[0].date);
                    pattern[idx].status = 'Absent';
                    nonFridayPresents = nonFridayPresents.slice(1);
                }
            }
        ...
        let students = classes.flatMap(cls => cls.students.map(student => ({
            ...
            absences: [
                ...
                { day: 'Friday', count: Math.floor(Math.random() * 4) + 1 } // Ensure at least 1 Friday absence
            ],
            consecutiveAbsences: Math.floor(Math.random() * 15),
            attendancePattern: generateMonthlyAttendancePattern(
                student.attendanceRate,
                Math.floor(Math.random() * 4) + 1 // Random Friday absences (1-4)
            ),
            factors: {
                late: {
        ...
        function updateData() {
            ...
            let year = 2025;
            let month = 6; // July 2025 (0-based index)
            ...
            if (startDateValue && endDateValue) {
                ...
                filteredStudents = filteredStudents.map(s => ({
                    ...
                    attendancePattern: generateMonthlyAttendancePattern(
                        s.attendanceRate * (Math.random() * 0.1 + 0.95),
                        s.absences.find(a => a.day === 'Friday')?.count || 0,
                        year,
                        month
                    )
                }));
                ...
                month = now.getMonth() - 1; // Last month
                if (month < 0) {
                    month = 11;
                    year--;
                }
                ...
                const multiplier = selectedTime === 'week' ? 0.98 : selectedTime === 'year' ? 0.90 : 1;
                filteredStudents = filteredStudents.map(s => ({
                    ...s,
                    attendanceRate: s.attendanceRate * multiplier,
                    attendancePattern: generateMonthlyAttendancePattern(
            ...
            // Update Monthly Attendance Patterns Table
            monthlyAttendance.innerHTML = '';
            filteredStudents.forEach(student => {
                const row = document.createElement('tr');
                ...
                const calendarHtml = `
                    <div class="monthly-attendance">
                        ...
                        <div class="calendar-grid">
                            ${student.attendancePattern.map(item => `
                                <div class="calendar-day">
                                    ${item.status === 'Empty' ? 
                                        `<span class="calendar-status calendar-empty"></span>` :
                                        `<span class="calendar-status attendance-${item.status.toLowerCase()}" title="${item.date}"></span>`
                                    }
                                </div>
                            `).join('')}
                        </div>
                ...
                row.innerHTML = `
                    ...
                    <td>${calendarHtml}</td>
                `;
                monthlyAttendance.appendChild(row);
            });
        ...
        function showStudentPrediction(student) {
            ...
            const chronicAbsenteeism = Math.min(100, ((student.consecutiveAbsences / 180) * 100));
            ...
            predictionDetails.innerHTML = `
                ...
                <div class="detail-item">
                    <strong>Probability of Being Present Tomorrow:</strong> ${probabilityPresentTomorrow.toFixed(0)}%
                </div>
                <div class="detail-item">
                    <strong>Chronic Absenteeism:</strong> ${chronicAbsenteeism.toFixed(0)}%
                </div>
            `;
            ...
            const analyticsData = [
                ...
                },
                {
                    ...
                    recommendation: student.absences[0]?.reason === 'Health Issue' ? 'Health check-up' : 'Address specific issue'
                },
                {
