...
ini_set('error_log', 'php-error.log'); // Specify log file path
...
$user = ['teacher_id' => $_SESSION['teacher_id']];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        if ($_POST['action'] === 'checkSubject') {
            ...
            exit;
        } elseif ($_POST['action'] === 'uploadPhoto') {
    ...
    try {
        if ($_GET['action'] === 'fetchClasses') {
            echo json_encode(fetchClassesForTeacher());
            exit;
        } elseif ($_GET['action'] === 'fetchStudents') {
...
<head>
    ...
    <style>
    /* Added CSS for Add Student Modal (similar to existing modals, with minor adjustments) */
...
#addStudentModal .modal-body {
    ...
    background: linear-gradient(180deg, #f9fafb, #ffffff);
    display: grid;
    grid-template-columns: 1fr 1fr; /* Two-column layout for fields */
    gap: var(--spacing-lg);
}
...
/* Responsive adjustments for Add Student Modal */
@media (max-width: 768px) {
    #addStudentModal .modal-body {
...
<body>
    ...
    <div id="addStudentModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="modal-title">Add New Student to Preview</h2>
            ...
            <form id="addStudentForm" class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="lrn" required placeholder="Enter LRN">
                </div>
                <div class="form-group">
                    <label class="form-label" for="last_name">Last Name *</label>
                    <input type="text" class="form-input" id="last_name" required placeholder="Enter Last Name">
                </div>
                <div class="form-group">
                    <label class="form-label" for="first_name">First Name *</label>
                    <input type="text" class="form-input" id="first_name" required placeholder="Enter First Name">
                </div>
                <div class="form-group">
                    <label class="form-label" for="middle_name">Middle Name</label>
                    <input type="text" class="form-input" id="middle_name" placeholder="Enter Middle Name">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="grade_level" placeholder="Enter Grade Level">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="parent_name" placeholder="Enter Parent Name">
                </div>
                <div class="form-group">
                    <label class="form-label" for="emergency_contact">Emergency Contact</label>
                    <input type="text" class="form-input" id="emergency_contact" placeholder="Enter Emergency Contact">
                </div>
                <div class="form-group">
                    <label class="form-label" for="photo">Photo</label>
                    <input type="file" class="form-input" id="photo" accept="image/*">
                </div>
            </form>
            <div class="form-actions">
                <button type="button" class="btn btn-secondary" onclick="closeAddStudentModal()">Cancel</button>
                <button type="button" class="btn btn-primary" onclick="saveNewStudent()">Save & Add to Preview</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function renderStudentTable(students, classId) {
            ...
            students.forEach(student => {
                ...
                const qrDisplay = qrSrc ? `<img src="${qrSrc}" alt="QR Code" style="max-width: 50px; max-height: 50px;"><br><small>${sanitizeHTML(student.qr_code)}</small>` : 'QR Code To Be Provided';
        ...
        function deleteStudent(classId, lrn) {
            ...
            fetch('manage-classes.php', {
                    ...
                    body: `action=deleteStudent&classId=${classId}&lrn=${encodeURIComponent(lrn)}`
                })
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Network response was not ok: ' + response.statusText);
        ...
        function openAddStudentModal() {
            ...
            if (addStudentModal) addStudentModal.classList.add('show');
        }
        ...
        function uploadPhoto(file) {
            return new Promise((resolve, reject) => {
                if (!file) {
                    resolve(null); // No photo uploaded, proceed without photo
                    return;
                }
                ...
                fetch('manage-classes.php', {
                    ...
                    return response.text().then(text => {
                        console.log('Raw photo upload response:', text);
                        try {
        ...
        function saveNewStudent() {
            const classId = document.getElementById('studentModal')?.dataset.classId;
            if (!classId) {
            ...
            const grade_level = document.getElementById('grade_level')?.value?.trim() || '';
            ...
            if (!lrn || !last_name || !first_name) {
                alert('LRN, Last Name, and First Name are required fields.');
                return;
            }
            ...
            const row = [
                ...
                parent_name,
                emergency_contact,
                null, // Placeholder for photo filename
                null  // Placeholder for QR code filename
            ];

            const photoPromise = photoFile ? uploadPhoto(photoFile) : Promise.resolve(null);

            photoPromise.then(photoFilename => {
                row[11] = photoFilename;
                const qrContent = `${lrn}, ${first_name}, ${last_name}${middle_name ? ' ' + middle_name : ''}`;
                const qrsToGenerate = [{ lrn: lrn, content: qrContent }];

                const formData = new FormData();
                formData.append('action', 'generateQRCodes');
                formData.append('qrs', JSON.stringify(qrsToGenerate));

                return fetch('manage-classes.php', {
                    method: 'POST',
                    body: formData,
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    ...
                    console.log('Response status:', response.status, 'OK:', response.ok);
                    return response.text().then(text => {
                        console.log('Raw QR code generation response:', text);
                        try {
                ...
                if (data.success && data.qr_files && data.qr_files[lrn]) {
                    row[12] = data.qr_files[lrn];
                    previewData.push(row);
                    renderPreviewTable();
                    closeAddStudentModal();
                    alert('Student added to preview successfully!');
                } else {
        ...
        function renderPreviewTable() {
            ...
            const previewTableContainer = document.getElementById('previewTableContainer');
            if (!tbody || !previewTableContainer) return;

            tbody.innerHTML = '';
            previewTableContainer.style.display = previewData.length > 0 ? 'block' : 'none';

            previewData.forEach((row, index) => {
                const photoSrc = row[11] ? `Uploads/${row[11]}` : '';
                const qrSrc = row[12] ? `qrcodes/${row[12]}` : '';
                const qrDisplay = qrSrc ? `<img src="${qrSrc}" alt="QR Code" style="max-width: 50px; max-height: 50px;"><br><small>${sanitizeHTML(row[12])}</small>` : 'QR Code To Be Provided';

                const tr = document.createElement('tr');
                tr.innerHTML = `
                    <td>${sanitizeHTML(row[0] || 'N/A')}</td>
                    <td>${sanitizeHTML(row[1] || 'N/A')}</td>
                    <td>${sanitizeHTML(row[2] || 'N/A')}</td>
                    <td>${sanitizeHTML(row[3] || 'N/A')}</td>
                    <td>${sanitizeHTML(row[4] || 'N/A')}</td>
                    <td>${sanitizeHTML(row[5] || 'N/A')}</td>
                    <td>${sanitizeHTML(row[6] || 'N/A')}</td>
                    <td>${sanitizeHTML(row[7] || 'N/A')}</td>
                    <td>${sanitizeHTML(row[8] || 'N/A')}</td>
                    <td>${sanitizeHTML(row[9] || 'N/A')}</td>
                    <td>${sanitizeHTML(row[10] || 'N/A')}</td>
                    <td>${photoSrc ? `<img src="${photoSrc}" alt="Student Photo" style="max-width: 45px; max-height: 45px; border-radius:50%;">` : 'Photo To Be Provided'}</td>
                    <td>${qrDisplay}</td>
                    <td>
                        <button class="btn btn-sm btn-danger" onclick="removeFromPreview(${index})">
                            <i class="fas fa-trash"></i>
                        </button>
                    </td>
                `;
        ...
        function removeFromPreview(index) {
            if (!confirm('Are you sure you want to remove this student from the preview?')) return;
