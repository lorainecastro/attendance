...
if (isset($_POST['bulk_delete']) && isset($_POST['lrns']) && isset($_POST['class_id'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT photo FROM students WHERE lrn = ?");
        $deleteStmt = $pdo->prepare("DELETE FROM class_students WHERE lrn = ? AND class_id = ?");
        foreach ($lrns as $lrn) {
            // Delete photo
            $stmt->execute([$lrn]);
            $student = $stmt->fetch();
            ...
            // Delete from class_students
            $deleteStmt->execute([$lrn, $class_id]);
        }
        $pdo->commit();
        echo json_encode(['success' => true]);
    } catch (PDOException $e) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'generateQR') {
    ...
    try {
        ...
        chmod($savePath, 0666); // Ensure QR code file is readable/writable
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['lrn']) && !isset($_POST['bulk_delete'])) {
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] == 0) {
        ...
        if (!file_exists($dir)) {
            mkdir($dir, 0777, true);
            chmod($dir, 0777); // Ensure directory is readable/writable
        }
        $path = $dir . '/' . $photo;
        ...
        chmod($path, 0666); // Ensure photo file is readable/writable
    }
